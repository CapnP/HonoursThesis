...
is \emph{subsumed} by an existing one (see Section \label{sec:terminology}).
...
\intertext{\tcent{Where there exists some \emph{matcher} $\sigma$ such that $(l \approx r)\sigma = s \approx t$\\ (i.e. $l \approx r$ \emph{subsumes} $s \approx t$).
...
If a substitution for the variables in $l \approx r$
...
Attempting to directly apply our existing Fingerprint Index to Negative Unit Simplification and
