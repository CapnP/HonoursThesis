...
Hashmap tree structure!!
 
\begin{listing}[H]
\begin{scalacode}
/** Algebraic Data type for our index. Either we are at a leaf (set of terms)
 *  or must continue traversal via the map (which should map to a new Index). */ 
...
\begin{listing}[H]
\begin{scalacode}
private def retrieveCompatible
...
However; this ability has been introduced at the cost of many grave errors which
(if not detected at this stage) 
...
\subsubsection{Non-unique Storage}
...
With \beagle\ now fully indexing terms for superposition
