...
is required. Provided here is a brief glossary of important FOL terminology used throughout the paper.

\subsubsection{Logic Expressions}

\begin{itemize}
\item \textbf{Variable:}
\item \textbf{Function Symbol:}
\item \textbf{Term:}
\item \textbf{Equation:}
\item \textbf{Literal:} An equation or a \emph{negated} equation, 
\item \textbf{Clause:} In this paper we use clause to refer specifically to a \emph{conjunctive normal form} clause,
which is a collection of literals joined with the logical OR operator.
\end{itemize}

\subsubsection{Logic Calculus}

A \emph{calculus} refers to a set of rules 
