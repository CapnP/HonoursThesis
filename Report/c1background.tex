...
computing which feature exists at each position. An example fingerprint computation
follows. In the example $\{f,g\}$ are functions, $\{a,b\}$ are
constants and $\{x,y\}$ are variables. See Section \label{sec:terminology} for an explanation of position syntax.
\[t = f(a, g(a, x, y), b)\]
